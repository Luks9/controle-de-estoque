...
header('Access-Control-Allow-Methods: GET, POST, PUT');
	...
	if ( isset( $_POST ) && ! empty( $_POST ) ) {
		...
		if ( $erro == false ) {
			...
			$pdo_insere = $conexao_pdo->prepare("INSERT INTO t_perda_materiais (id_instalacao, id_tecnico, conectores, conectores2) VALUES (?, ?, ?, ?);");
			$pdo_insere->execute( array($id_instalacao, $id_tecnico, $pConector, $pConector2) );

			if ($_POST['onu'] > 0) {
				for ($i=0; $i < $_POST['onu']; $i++) {
					$vlo ='onu_select' . $i;
					$pdo_insere = $conexao_pdo->prepare("UPDATE t_estoque_serializados SET status = false, id_instalacao = '$id_instalacao' ,nome_cliente = '" . $nome_cliente . "' WHERE serial = '" . $_POST[$vlo] . "';");
					$pdo_insere->execute();
			...
			if ($_POST['stb'] > 0) {
				for ($i=0; $i < $_POST['stb']; $i++) {
					$vls ='stb_select' . $i;
					$pdo_insere = $conexao_pdo->prepare("UPDATE t_estoque_serializados SET status = false, id_instalacao = '$id_instalacao' , nome_cliente = '" . $nome_cliente . "' WHERE serial = '" . $_POST[$vls] . "';");
					$pdo_insere->execute();
				}
			}
			if ($_POST['telefones'] > 0) {
				for ($i=0; $i < $_POST['telefones']; $i++) {
					...
					$pdo_insere = $conexao_pdo->prepare("UPDATE t_estoque_serializados SET status = false, id_instalacao = '$id_instalacao' , nome_cliente = '" . $nome_cliente . "' WHERE serial = '" . $_POST[$vlt] . "';");
					$pdo_insere->execute();
				}
			}
			if ($_POST['radio'] > 0) {
				for ($i=0; $i < $_POST['radio']; $i++) {
					$vlr ='radio_select' . $i;
					$pdo_insere = $conexao_pdo->prepare("UPDATE t_estoque_serializados SET status = false, id_instalacao = '$id_instalacao', nome_cliente = '" . $nome_cliente . "' WHERE serial = '" . $_POST[$vlr] . "';");
					$pdo_insere->execute();
				}
			}
		}
